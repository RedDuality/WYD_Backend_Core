...
public class UpdateThisAndAllFollowingTests {
  ...
  public async Task UpdateAllTheSequenceGeneratedFirstInstance_ShouldThrow() {
    var startTime = DateTimeOffset.UtcNow.AddHours(1);
  ...
  public async Task UpdateAllTheSequenceGeneratedNthInstance_ShouldSucceed_WithNewTitle() {
    ...
    var newEvent = detachedEvents.First();
    ...
    var detachedInstance2 = oldDetachedList.Instances.First();
  ...
  public async Task UpdateAllTheSequenceGeneratedFirstInstance_ShouldSucceed_WithBothTimeUpdate() {
    ...
    detachedEvents.Count.Should().Be(oldoldDetachedList.Instances.Count);
    ...
    foreach (var de in detachedEvents) {
      ...
      oldDetachedList.Instances.Count(i => i.EventId == de.Id).Should().Be(1);
      oldDetachedList.Instances.First(i => i.EventId == de.Id).StartTime.Should().Be(de.StartTime);
      oldDetachedList.Instances.First(i => i.EventId == de.Id).RecurrencyId.Should().Be(de.RecurrencyInstanceId);
    }
  }
